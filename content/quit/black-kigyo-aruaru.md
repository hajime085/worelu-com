---
title: "ブラック企業あるある20選｜あなたの会社は大丈夫？診断付きでチェック"
...
<button class="aa-result-btn" onclick="calcAaResult()">ブラック耐性レベルを診断する →</button>

<div id="aa-result" style="display:none"></div>
...
.aa-share-row { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 16px; }
.aa-dl-btn { flex: 1; min-width: 140px; background: #fff; color: #2563EB; border: 2px solid #2563EB; border-radius: 8px; padding: 10px 16px; font-size: 13px; font-weight: 700; cursor: pointer; font-family: inherit; transition: all 0.2s; }
.aa-dl-btn:hover { background: #EFF6FF; }
.aa-share-btn { flex: 1; min-width: 140px; text-align: center; }
...
function downloadShindan(level) {
  var img = new Image();
  ...
  img.onload = function() {
    ...
    a.download = 'worelu_black_level' + level + '.png';
    a.href = canvas.toDataURL('image/png');
    a.click();
  };
  img.onerror = function() {
    // crossOrigin失敗時は直接ダウンロード
    var a = document.createElement('a');
    a.download = 'worelu_black_level' + level + '.png';
    a.href = '/images/shindan/black_lv00' + level + '.png';
    a.click();
  };
  img.src = '/images/shindan/black_lv00' + level + '.png';
}

function calcAaResult() {
  var score = aaCounts.wakaru;
  var lvNum, levelNum, levelLabel, subCopy, title, body, color, bg, badge;

  if (score <= 3) {
    lvNum = 1; levelNum = 'LEVEL 1'; levelLabel = '正常な感覚の持ち主'; subCopy = 'ブラック企業度：低';
    title = 'ブラック企業とは無縁の職場にいるようです';
    body = score + '個しか当てはまりませんでした。今の職場は比較的健全な可能性があります。この感覚を忘れずに、引き続き自分の状態を観察してください。';
    color = '#15803D'; bg = '#F0FDF4'; badge = '[ OK ]';
  } else if (score <= 7) {
    lvNum = 2; levelNum = 'LEVEL 2'; levelLabel = '感覚が麻痺し始めています'; subCopy = 'ブラック企業度：注意';
    title = 'ちょっと待って。感覚が麻痺し始めているかもしれません';
    body = score + '個当てはまりました。「これくらい普通かな」と思い始めていたら要注意。外の世界の基準と比較してみましょう。';
    color = '#1D4ED8'; bg = '#EFF6FF'; badge = '[ ! ]';
  } else if (score <= 12) {
    lvNum = 3; levelNum = 'LEVEL 3'; levelLabel = 'その当たり前、本当に普通ですか？'; subCopy = 'ブラック企業度：高';
    title = 'その「当たり前」、本当に当たり前ですか？';
    body = score + '個も当てはまりました。感覚がかなり麻痺している可能性があります。転職を含めた選択肢を真剣に検討する時期かもしれません。';
    color = '#C2410C'; bg = '#FFF7ED'; badge = '[ !! ]';
  } else {
    lvNum = 4; levelNum = 'LEVEL 4'; levelLabel = 'あなたの感覚はかなり麻痺しています'; subCopy = 'ブラック企業度：危険';
    title = 'あなたの感覚はかなり麻痺しています';
    body = score + '個当てはまりました。ここまで多いと心身への影響が出ている可能性があります。あなたの感覚は正しい。逃げることは恥ではありません。';
    color = '#991B1B'; bg = '#FEF2F2'; badge = '[ SOS ]';
  }
  ...
  var checkedItems = [];
  document.querySelectorAll('.aa-item.wakaru .aa-text').forEach(function(el) {
    if (checkedItems.length < 3) checkedItems.push(el.textContent.trim().slice(0, 20) + '…');
  });

  var links =
    '<div class="aa-next-links">' +
    '<div class="aa-next-title">次に読む</div>' +
    '<a href="/articles/quit/black-kigyo-shindan/" class="aa-next-link">ブラック企業診断（15問スコアリング）</a>' +
    '<a href="/articles/quit/yameru-yuuki/" class="aa-next-link">仕事を辞める勇気が出ない人へ</a>' +
    '<a href="/articles/quit/taisyoku-daikou-moumuri/" class="aa-next-link">退職代行を使うべき状況とは</a>' +
    '<a href="/stress-check/" class="aa-next-link">AIストレスチェック（無料・約1分）</a>' +
    '</div>';

  var html =
    '<div class="aa-result-box" style="background:' + bg + ';border-color:' + color + ';color:' + color + '">' +
    '<div class="aa-result-badge-row"><span class="aa-result-badge">' + badge + '</span><span class="aa-result-sub">' + subCopy + '</span></div>' +
    '<div class="aa-result-level">' + levelNum + '</div>' +
    '<div class="aa-result-title">' + levelLabel + '</div>' +
    '<div class="aa-result-body" style="color:#334155">' + body + '</div>' +
    '<div class="aa-share-row">' +
    '<button onclick="downloadShindan(' + lvNum + ')" class="aa-dl-btn">結果画像をダウンロード</button>' +
    '<a href="https://twitter.com/intent/tweet?text=' + shareText + '" target="_blank" rel="noopener" class="aa-share-btn">X（旧Twitter）にシェアする</a>' +
    '</div>' +
    '</div>' +
    links;
